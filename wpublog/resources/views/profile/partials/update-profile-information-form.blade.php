<section>
    ...
    <form method="post" action="{{ route('profile.update') }} " class="mt-6 space-y-6" enctype="multipart/form-data">
        ...
        <div>
            <label class="block mb-2.5 text-sm font-medium text-gray-900 dark:text-white" for="avatar">Upload
                avatar</label>
            <input
                class="@error('avatar')
        ...
        <div>
            <img class="w-20 h-20 rounded-full"
                src="{{ $user->avatar ? asset('storage/' . $user->avatar) : asset('img/avatar.png') }}" alt="{{ $user->name }}" id="avatar-preview">
        </div>
